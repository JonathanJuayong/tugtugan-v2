...
{:else if item.chords === '' && item.lyrics.length > 0}
    <pre class={["grid content-end transition-all", item.isHighlighted ? highlight : '']} {id}><button {onclick} class={"cursor-pointer whitespace-pre-wrap"}>{item.lyrics}</button></pre>
{:else if item.lyrics === '' && item.chords.length > 0}
    <pre data-chord={item.chords} class={["grid content-start transition-all", item.isHighlighted ? highlight : '']} {id}><button {onclick} class="cursor-pointer whitespace-pre-wrap">{item.chords}</button></pre>
{:else}
